...
public class RecipeController {
    ...
    public String doAddRecipe(@Valid AddRecipeDTO data, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        boolean recipeAdded = recipeService.createRecipe(data);
    ...
    @PostMapping("/add-to-favourites/{recepeId}")
    public String addToFavourites(@PathVariable long recepeId) {
        if (!userSession.isLoggedIn()) {
            return "redirect:/";
        }

        recipeService.addToFavourites(userSession.id(), recepeId);
        return "redirect:/home";
